...
class Decoder(nn.Module):
  def __init__(self, input_size_decoder,input_size_encoder,input_size_ci,hidden_size,
    ...
    self.iL1 = self.input_size_decoder + self.ci
  ...
  def forward(self, input, h_n, enc_outputs,input_ci,mask):
    # CLINICAL DATA
    input_ci = input_ci.unsqueeze(1)
    input  = torch.cat((input,input_ci),dim=-1)
...
class Modelo(nn.Module):
  ...
  def forward(self, input_tensor,input_ci, target_tensor,mask,teacher_forcing_ratio = 0.01):
    ...
    for t in range(0, target_len):
      decoder_output, decoder_hidden, att_weight = self.decoder(decoder_input, decoder_hidden,enc_output,input_ci,mask[:,t])
